...
public class TransactionService {
     ...
     * 계좌가 이미 해지 상탱니 경우, 거래 금액이 잔액보다 큰 경우,
    ...
    private void validateUseBalance(AccountUser user, Account account, Long amount) {
        if (!Objects.equals(user.getId(), account.getAccountUser().getId())) { //사용자와 계좌의 소유주가 다른 경우
            throw new AccountException(ErrorCode.USER_ACCOUNT_UN_MATCH);
        }
        if (account.getAccountStatus() != AccountStatus.IN_USE) { //이미 해지된 계좌인 경우
            throw new AccountException(ErrorCode.ACCOUNT_ALREADY_UNREGISTERED);
        }
        if (account.getBalance() < amount) { //사용금액이 잔액보다 많은 경우
    ...
    public TransactionDto cancelBalance(
        ...
        return TransactionDto.fromEntity(saveAndGetTransaction(
                TransactionType.CANCEL, TransactionResultType.S, account, amount));
    }
